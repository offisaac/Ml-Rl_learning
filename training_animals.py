...
my_train_data_loader=torch.utils.data.DataLoader(my_train_data,batch_size=1000,shuffle=True)
my_test_data_loader=torch.utils.data.DataLoader(my_test_data,batch_size=1000,shuffle=True)
class CNNClassifier(nn.Module):
    def __init__(self,class_num):
        super().__init__()#!外部的输入不应该放在类名后(作为继承) 而是放在init后
        self.conv1=nn.Conv2d(3,64,3,stride=1,padding=1)#这里padding=1是指每个边都向外扩一个"0"的像素 所以对于行 相当于扩了两个像素
        self.pool=nn.MaxPool2d(2,2)
        self.conv2=nn.Conv2d(64,32,3,stride=1,padding=1)#实际设计图片的卷积层时 通道输出图层数由filter决定 最后给线性层输入的列向量行数字需要根据卷积后的图片尺寸决定 这个有公式计算
        #卷积操作影响图层个数和图的尺寸 池化只影响图的尺寸 图最终尺寸只和初始图尺寸 卷积的stride padding有关 和通道无关 通道只和图层有关
        self.func1=nn.Linear(32*8*8,128)#怎么从32通道8*8array对应到32*8*8个列向量 底层不用考虑
        self.func2=nn.Linear(128,class_num)
        self.relu=nn.ReLU()
...
#         break
# 保存模型参数 直接保存到指定地址即可
# 导入模型餐宿 需要创建模型 导入模型参数 模型进行eval处理
